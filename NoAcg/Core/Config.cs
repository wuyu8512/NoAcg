...
namespace NoAcg.Core
{
    internal class Config
    {
        ...
        {
            foreach (var c in config.UniversalConfigs.Where(c => c.Param != null))
            {
                c.Param = c.Param.Select(GetValue).ToArray();
            }

            foreach (var c in config.PrivateConfigs.Where(c => c.Param != null))
            {
                c.Param = c.Param.Select(GetValue).ToArray();
            }

            foreach (var c in config.GroupConfigs.Where(c => c.Param != null))
            {
                c.Param = c.Param.Select(GetValue).ToArray();
            }
        }
